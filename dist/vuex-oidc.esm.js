...
var createReactiveState = (function (oidcSettings) {
  var stateSettings = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var oidcEventListeners = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  var oidcConfig = getOidcConfig(oidcSettings);
  var oidcUserManager = createOidcUserManager(oidcSettings);
  ...
  var oidcCallbackPath = getOidcCallbackPath(oidcConfig.redirect_uri, stateSettings.routeBase || '/');
  var oidcPopupCallbackPath = getOidcCallbackPath(oidcConfig.popup_redirect_uri, stateSettings.routeBase || '/');
  var oidcSilentCallbackPath = getOidcCallbackPath(oidcConfig.silent_redirect_uri, stateSettings.routeBase || '/'); // Add event listeners passed into factory function
  ...
  var fn = {
    isAuthenticated: function isAuthenticated(state) {
      return !!state[stateSettings.isAuthenticatedBy];
    },
    routeIsOidcCallback: function routeIsOidcCallback(route) {
      if (route.meta && route.meta.isOidcCallback) {
        return true;
      }
    ...
    routeIsPublic: function routeIsPublic(route) {
      if (route.meta && route.meta.isPublic) {
        return true;
      }
      ...
      if (stateSettings.publicRoutePaths && stateSettings.publicRoutePaths.map(function (path) {
        return path.replace(/\/$/, '');
      }).indexOf(route.path.replace(/\/$/, '')) > -1) {
        return true;
      }
    ...
    errorPayload: function errorPayload(context, error) {
      return {
        context: context,
        error: error && error.message ? error.message : error
      };
    },
    dispatchCustomErrorEvent: function dispatchCustomErrorEvent(eventName, payload) {
      // oidcError and automaticSilentRenewError are not UserManagement events, they are events implemeted in vuex-oidc,
      if (typeof oidcEventListeners[eventName] === 'function') {
        oidcEventListeners[eventName](payload);
      }
  ...
  var getters = {
    oidcIsAuthenticated: computed(function () {
      return isAuthenticated(state);
    }),
    oidcUser: computed(function () {
      return state.user;
    }),
    oidcAccessToken: computed(function () {
      return tokenIsExpired(state.access_token) ? null : state.access_token;
    }),
    oidcAccessTokenExp: computed(function () {
      return tokenExp(state.access_token);
    }),
    oidcScopes: computed(function () {
      return state.scopes;
    }),
    oidcIdToken: computed(function () {
      return tokenIsExpired(state.id_token) ? null : state.id_token;
    }),
    oidcIdTokenExp: computed(function () {
      return tokenExp(state.id_token);
    }),
    oidcRefreshToken: computed(function () {
      return tokenIsExpired(state.refresh_token) ? null : state.refresh_token;
    }),
    oidcRefreshTokenExp: computed(function () {
      return tokenExp(state.refresh_token);
    }),
    oidcAuthenticationIsChecked: computed(function () {
      return state.is_checked;
    }),
    oidcError: computed(function () {
      return state.error;
    })
  };
  var mutations = {
    setOidcAuth: function setOidcAuth(user) {
      state.id_token = user.id_token;
      state.access_token = user.access_token;
      state.refresh_token = user.refresh_token;
      state.user = user.profile;
    ...
    setOidcUser: function setOidcUser(user) {
      state.user = user ? user.profile : null;
    },
    unsetOidcAuth: function unsetOidcAuth() {
      state.id_token = null;
      state.access_token = null;
      state.refresh_token = null;
      state.user = null;
    },
    setOidcAuthIsChecked: function setOidcAuthIsChecked() {
      state.is_checked = true;
    },
    setOidcEventsAreBound: function setOidcEventsAreBound() {
      state.events_are_bound = true;
    },
    setOidcError: function setOidcError(payload) {
      state.error = payload.error;
      dispatchCustomErrorEvent('oidcError', payload);
    }
  };

  var authenticateOidcSilent = function authenticateOidcSilent() {
    var payload = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    // Take options for signinSilent from 1) payload or 2) storeSettings if defined there
    var options = payload.options || stateSettings.defaultSigninSilentOptions || {};
    return new Promise(function (resolve, reject) {
      oidcUserManager.signinSilent(options).then(function (user) {
  ...
  var oidcCheckAccess = function oidcCheckAccess(route) {
    return new Promise(function (resolve) {
      ...
      var isAuthenticatedInStore = fn.isAuthenticated(state);
      getUserPromise.then(function (user) {
        if (!user || user.expired) {
          var authenticateSilently = oidcConfig.silent_redirect_uri && oidcConfig.automaticSilentSignin;
  ...
  var authenticateOidc = function authenticateOidc() {
    var payload = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    ...
    var options = payload.options || stateSettings.defaultSigninRedirectOptions || {};
    return oidcUserManager.signinRedirect(options).catch(function (err) {
      mutations.setOidcError(fn.errorPayload('authenticateOidc', err));
    });
  };

  var oidcSignInCallback = function oidcSignInCallback(url) {
    return new Promise(function (resolve, reject) {
      oidcUserManager.signinRedirectCallback(url).then(function (user) {
        oidcWasAuthenticated(user);
        resolve(sessionStorage.getItem('vuex_oidc_active_route') || '/');
  ...
  var authenticateOidcPopup = function authenticateOidcPopup() {
    var payload = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    // Take options for signinPopup from 1) payload or 2) storeSettings if defined there
    var options = payload.options || stateSettings.defaultSigninPopupOptions || {};
    return oidcUserManager.signinPopup(options).then(function (user) {
      oidcWasAuthenticated(user);
    }).catch(function (err) {
  ...
  var oidcSignInPopupCallback = function oidcSignInPopupCallback(url) {
    return new Promise(function (resolve, reject) {
      oidcUserManager.signinPopupCallback(url).catch(function (err) {
  ...
  var oidcWasAuthenticated = function oidcWasAuthenticated(user) {
    mutations.setOidcAuth(user);

    if (!state.events_are_bound) {
      oidcUserManager.events.addAccessTokenExpired(function () {
        return mutations.unsetOidcAuth();
      });

      if (oidcSettings.automaticSilentRenew) {
        oidcUserManager.events.addAccessTokenExpiring(function () {
          authenticateOidcSilent().catch(function (err) {
            dispatchCustomErrorEvent('automaticSilentRenewError', fn.errorPayload('authenticateOidcSilent', err));
          });
        });
      }
  ...
  var storeOidcUser = function storeOidcUser(user) {
    return oidcUserManager.storeUser(user).then(function () {
      return oidcUserManager.getUser();
    }).then(function (user) {
      return oidcWasAuthenticated(user);
    }).then(function () {}).catch(function (err) {
  ...
  var getOidcUser = function getOidcUser() {
    return oidcUserManager.getUser().then(function (user) {
  ...
  var addOidcEventListener = function addOidcEventListener(payload) {
    addUserManagerEventListener(oidcUserManager, payload.eventName, payload.eventListener);
  };

  var removeOidcEventListener = function removeOidcEventListener(payload) {
    removeUserManagerEventListener(oidcUserManager, payload.eventName, payload.eventListener);
  };

  var signOutOidc = function signOutOidc(payload) {
    return oidcUserManager.signoutRedirect(payload).then(function () {
      mutations.unsetOidcAuth();
    });
  };

  var signOutOidcCallback = function signOutOidcCallback() {
    return oidcUserManager.signoutRedirectCallback();
  };

  var signOutPopupOidc = function signOutPopupOidc(payload) {
    return oidcUserManager.signoutPopup(payload).then(function () {
      mutations.unsetOidcAuth();
    });
  };

  var signOutPopupOidcCallback = function signOutPopupOidcCallback() {
    return oidcUserManager.signoutPopupCallback();
  };

  var signOutOidcSilent = function signOutOidcSilent(payload) {
    return new Promise(function (resolve, reject) {
      try {
        oidcUserManager.getUser().then(function (user) {
          var args = objectAssign([payload || {}, {
  ...
  var removeOidcUser = function removeOidcUser() {
    return oidcUserManager.removeUser().then(function () {
      mutations.unsetOidcAuth();
    });
  };

  var clearStaleState = function clearStaleState() {
    return oidcUserManager.clearStaleState();
  };

  var oidcIsRoutePublic = function oidcIsRoutePublic() {
    return function (route) {
      return fn.routeIsPublic(route);
    };
  };

  var methods = {
    authenticateOidcSilent: authenticateOidcSilent,
    oidcCheckAccess: oidcCheckAccess,
    ...
    oidcWasAuthenticated: oidcWasAuthenticated,
    storeOidcUser: storeOidcUser,
    getOidcUser: getOidcUser,
    addOidcEventListener: addOidcEventListener,
    removeOidcEventListener: removeOidcEventListener,
    signOutOidc: signOutOidc,
    signOutOidcCallback: signOutOidcCallback,
    signOutPopupOidc: signOutPopupOidc,
    signOutPopupOidcCallback: signOutPopupOidcCallback,
    signOutOidcSilent: signOutOidcSilent,
    removeUser: removeUser,
    removeOidcUser: removeOidcUser,
    clearStaleState: clearStaleState,
    oidcIsRoutePublic: oidcIsRoutePublic
  };

  var reactiveState = _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, stateSettings), toRefs(state)), getters), methods);
...
var createReactiveStateRouterMiddleware = (function (state) {
  return function (to, from, next) {
    state.oidcCheckAccess(to).then(function (hasAccess) {
      if (hasAccess) {
        next();
      }
    });
...
var vuexOidcCreateReactiveState = createReactiveState;
...
var vuexOidcCreateReactiveStateRouterMiddleware = createReactiveStateRouterMiddleware;
...
export { vuexDispatchCustomBrowserEvent, vuexOidcCreateNuxtRouterMiddleware, vuexOidcCreateReactiveState, vuexOidcCreateReactiveStateRouterMiddleware, vuexOidcCreateRouterMiddleware, vuexOidcCreateStoreModule, vuexOidcCreateUserManager, vuexOidcGetOidcCallbackPath, vuexOidcProcessSignInCallback, vuexOidcProcessSilentSignInCallback, vuexOidcUtils };
